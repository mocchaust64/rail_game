extends Node

const SAVE_PATH := "user://flow_factory_save_v2.json"
const TEMP_PATH := "user://flow_factory_save_v2.tmp"
const BACKUP_PATH := "user://flow_factory_save_v2.bak"
const SAVE_VERSION := 3

# Player-facing accessibility bounds.
const SPEED_OPTIONS := [0.75, 1.0]
const LARGE_TEXT_SCALE := 1.25

var highest_unlocked_level: int = 1
var audio_enabled: bool = true
var haptic_enabled: bool = true
var tutorial_seen: bool = false
var reduced_motion: bool = false
var large_text: bool = false
var game_speed: float = 1.0


func _ready() -> void:
    load_save()


func load_save() -> void:
    var data := _read_save(SAVE_PATH)
    if data.is_empty():
        # The live file is missing or was left half-written. The backup is the
        # previous complete save, so recovering from it costs at most one save.
        data = _read_save(BACKUP_PATH)
        if data.is_empty():
            return
        _apply(data)
        # Repair the live file. save() deliberately refuses to promote a corrupt
        # live file into BACKUP_PATH, so the known-good recovery copy survives.
        save()
        return
    _apply(data)


func _read_save(path: String) -> Dictionary:
    if not FileAccess.file_exists(path):
        return {}
    var file := FileAccess.open(path, FileAccess.READ)
    if file == null:
        return {}
    var text := file.get_as_text()
    file.close()
    var json := JSON.new()
    if json.parse(text) != OK:
        return {}
    var parsed: Variant = json.data
    if typeof(parsed) != TYPE_DICTIONARY:
        return {}
    return parsed as Dictionary


func _apply(data: Dictionary) -> void:
    highest_unlocked_level = maxi(1, int(data.get("highest_unlocked_level", 1)))
    audio_enabled = bool(data.get("audio_enabled", true))
    haptic_enabled = bool(data.get("haptic_enabled", true))
    tutorial_seen = bool(data.get("tutorial_seen", false))
    reduced_motion = bool(data.get("reduced_motion", false))
    large_text = bool(data.get("large_text", false))
    game_speed = clampf(float(data.get("game_speed", 1.0)), 0.5, 1.0)


func unlock_level(level_number: int) -> void:
    highest_unlocked_level = max(highest_unlocked_level, level_number)
    save()


func mark_tutorial_seen() -> void:
    if tutorial_seen:
        return
    tutorial_seen = true
    save()


func set_audio_enabled(value: bool) -> void:
    audio_enabled = value
    save()


func set_haptic_enabled(value: bool) -> void:
    haptic_enabled = value
    save()


func set_reduced_motion(value: bool) -> void:
    reduced_motion = value
    save()


func set_large_text(value: bool) -> void:
    large_text = value
    save()


func set_game_speed(value: float) -> void:
    game_speed = clampf(value, 0.5, 1.0)
    save()


# UI font multiplier for the large-text setting.
func text_scale() -> float:
    return LARGE_TEXT_SCALE if large_text else 1.0


# Cargo speed after the player's speed preference is applied.
func scaled_speed(base_speed: float) -> float:
    return base_speed * game_speed


func reset_progress() -> void:
    highest_unlocked_level = 1
    tutorial_seen = false
    save()


func save() -> void:
    var data := {
        "version": SAVE_VERSION,
        "highest_unlocked_level": highest_unlocked_level,
        "audio_enabled": audio_enabled,
        "haptic_enabled": haptic_enabled,
        "tutorial_seen": tutorial_seen,
        "reduced_motion": reduced_motion,
        "large_text": large_text,
        "game_speed": game_speed,
    }

    # Stage and read back the payload before touching either durable copy.
    var file := FileAccess.open(TEMP_PATH, FileAccess.WRITE)
    if file == null:
        push_error("save aborted: could not open temporary save")
        return
    file.store_string(JSON.stringify(data))
    file.flush()
    file.close()

    if _read_save(TEMP_PATH).is_empty():
        push_error("save aborted: the temporary file did not read back as valid JSON")
        return

    var live_absolute := ProjectSettings.globalize_path(SAVE_PATH)
    var temp_absolute := ProjectSettings.globalize_path(TEMP_PATH)
    var backup_absolute := ProjectSettings.globalize_path(BACKUP_PATH)

    # Only a VALID live save is allowed to replace the backup. This matters on
    # recovery: copying a corrupt live file over the known-good .bak would turn a
    # one-file failure into permanent progress loss.
    var live_is_valid := not _read_save(SAVE_PATH).is_empty()
    if live_is_valid:
        if FileAccess.file_exists(BACKUP_PATH):
            DirAccess.remove_absolute(backup_absolute)
        var backup_error := DirAccess.copy_absolute(live_absolute, backup_absolute)
        if backup_error != OK:
            push_error("save aborted: could not preserve previous save as backup")
            return

    # Replace the live file only after the staged payload and, when applicable,
    # its previous backup are known-good. There is always at least one complete
    # durable copy during this transition.
    if FileAccess.file_exists(SAVE_PATH):
        var remove_error := DirAccess.remove_absolute(live_absolute)
        if remove_error != OK:
            push_error("save aborted: could not replace live save")
            return

    var rename_error := DirAccess.rename_absolute(temp_absolute, live_absolute)
    if rename_error != OK:
        push_error("save aborted: could not promote temporary save")
        return

    # The very first save has no predecessor. Seed BACKUP_PATH from the newly
    # verified live file so recovery protection exists immediately, not only
    # after the player's second save.
    if not FileAccess.file_exists(BACKUP_PATH):
        var seed_error := DirAccess.copy_absolute(live_absolute, backup_absolute)
        if seed_error != OK:
            push_warning("save completed but initial backup could not be created")
