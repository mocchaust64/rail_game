...
# continuous dark roadbed, fixed metal side rails and a moving centre channel.

const BASE_WIDTH := 0.92
const BED_WIDTH := 0.70
const RAIL_OFFSET := 0.36
const RAIL_WIDTH := 0.085
const SUPPORT_SPACING := 0.42
...
static func create_path(parent: Node3D, points: PackedVector3Array, trim_start: float = 0.0, trim_end: float = 0.0) -> Node3D:
    ...
    _support_layer(root, visible_points)
    _bolt_layer(root, visible_points)

    var motion := TrackMotion.new()
    motion.name = "TrackMotion"
    root.add_child(motion)
    motion.setup(visible_points)
    return root
...
static func _support_layer(parent: Node3D, points: PackedVector3Array) -> void:
    var total := TrackGeometry.length(points)
    var count := maxi(1, int(floor(total / SUPPORT_SPACING)))

    var mesh := BoxMesh.new()
    mesh.size = Vector3.ONE
    mesh.material = VisualFactory.material(Color("#676A6C"), 0.56, 0.0, 0.06)
    ...
    for i in range(count):
        ...
        var basis := Basis(Vector3.UP, angle).scaled(Vector3(0.78, 0.028, 0.055))
        multi.set_instance_transform(i, Transform3D(basis, p + Vector3(0, 0.145, 0)))

    var instance := MultiMeshInstance3D.new()
    instance.name = "Supports"
...
static func _bolt_layer(parent: Node3D, points: PackedVector3Array) -> void:
    var total := TrackGeometry.length(points)
    var support_count := maxi(1, int(floor(total / SUPPORT_SPACING)))
    ...
    multi.transform_format = MultiMesh.TRANSFORM_3D
    multi.mesh = mesh
    multi.instance_count = support_count * 2

    var index := 0
    for i in range(support_count):
        var distance := (float(i) + 0.5) * total / float(support_count)
        var sample := TrackGeometry.sample_distance(points, distance)
        var p: Vector3 = sample["position"]
        var tangent: Vector3 = sample["tangent"]
        var side := Vector3(tangent.z, 0.0, -tangent.x).normalized()
